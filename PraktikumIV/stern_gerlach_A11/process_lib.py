...
def output_constant(name):
    load_gnuplot_dict(gnuplot_data, gnuplot_dict)
    constant = gnuplot_dict[name]
    with open('output/c_%s.tex'%name, 'w') as ofile:
        ofile.writelines(print_nice(constant).replace('$', ''))
